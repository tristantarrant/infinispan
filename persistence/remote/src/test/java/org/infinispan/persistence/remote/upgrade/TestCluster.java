...
class TestCluster {
   ...
   <K, V> RemoteCache<K, V> getRemoteCache(String cacheName, MediaType mediaType) {
      ...
      Marshaller marshaller = switch (mediaType.toString()) {
         case MediaType.TEXT_PLAIN_TYPE -> new UTF8StringMarshaller();
         case MediaType.APPLICATION_SERIALIZED_OBJECT_TYPE -> new JavaSerializationMarshaller();
         default -> new ProtoStreamMarshaller();
      };
      ...
      static class CacheDefinitionBuilder {
         ...
         private CacheDefinitionBuilder addNewCache() {
            ...
            if (remotePort != null) {
               ...
               store.remoteCacheName(name).protocolVersion(protocolVersion).shared(true);

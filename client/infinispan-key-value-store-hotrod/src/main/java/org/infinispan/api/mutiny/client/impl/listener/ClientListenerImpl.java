package org.infinispan.api.mutiny.client.impl.listener;
...
public class ClientListenerImpl<K, V> implements Publisher {
   ...
   private final ClientKeyValueListener listener;
   private final ClassAllowList whitelist = new ClassAllowList(Collections.singletonList(".*"));

   public ClientListenerImpl(RemoteCache cache, ClientKeyValueListener listener) {
      this.cache = cache;

...
public class ConfigurationBuilder implements ConfigurationChildBuilder, Builder<Configuration> {
   ...
   public ConfigurationBuilder transactionTimeout(long timeout, TimeUnit timeUnit) {
      transactionTimeout = timeUnit.toMillis(timeout);
      return this;
   }
   ...
   public ConfigurationBuilder withProperties(Properties properties) {
      ...
      if (typed.containsKey(ConfigurationProperties.SERVER_FAILURE_TIMEOUT)) {
         this.serverFailureTimeout((int) typed.getDurationProperty(ConfigurationProperties.SERVER_FAILURE_TIMEOUT, serverFailureTimeout, true));
      }
      ...
      if (typed.containsKey(ConfigurationProperties.TRANSACTION_TIMEOUT)) {
         this.transactionTimeout = typed.getLongProperty(ConfigurationProperties.TRANSACTION_TIMEOUT, ConfigurationProperties.DEFAULT_TRANSACTION_TIMEOUT, true);
      }
   ...
   public Configuration create() {
      ...
      if (!this.servers.isEmpty()) {
         ...
         servers.add(new ServerConfigurationBuilder(this).host("127.0.0.1").port(ConfigurationProperties.DEFAULT_HOTROD_PORT).create());
      }

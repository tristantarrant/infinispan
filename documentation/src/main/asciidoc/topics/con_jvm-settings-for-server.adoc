...
You can also append additional JVM options with the `JAVA_OPTIONS` environment variable.
These options are added after the category variables.

[IMPORTANT]
====
If you are running {brandname} in a container, do not set `-Xmx` or `-Xms` because the values are automatically calculated from the container settings to use 70% of the container size.

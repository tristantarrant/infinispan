...
against `BasicCache` and `BasicCacheContainer` rather than `AsyncCache` and `CacheContainer`.
...
The `Flag` and `ClassLoader` API has changed. In the past, the following would work:

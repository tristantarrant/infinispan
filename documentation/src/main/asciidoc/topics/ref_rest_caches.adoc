...
= Re-indexing Data
Re-index all data in caches with `POST` requests and the
...
* `async` returns `204 (No Content)` immediately and the re-indexing operation continues running in the cluster. You can check the status with the link:#rest_v2_query_index_stats[Index Statistics] REST call.

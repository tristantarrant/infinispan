...
Generate JVM heap dumps for {brandname} servers with `POST` requests.
...
{brandname} generates a heap dump file in HPROF format in the server data directory and responds with the full path of the file in JSON format.

...
public class CounterParser implements ConfigurationParser {
   ...
   public void readElement(ConfigurationReader reader, ConfigurationBuilderHolder holder) {
      ...
      readElement(reader, builder.addModule(CounterManagerConfigurationBuilder.class));
   }

   public void readElement(ConfigurationReader reader, CounterManagerConfigurationBuilder builder) {
      ...
      switch (element) {
         case STRONG_COUNTER:
            ...
            if (!schema.since(10, 0)) {
               parseStrongCounterLegacy(reader, builder.addStrongCounter().name(item.getKey()));
            } else {
               parseStrongCounter(reader, builder.addStrongCounter().name(item.getKey()));
            }
            reader.endMapItem();
            break;
         case WEAK_COUNTER:
            parseWeakCounter(reader, builder.addWeakCounter().name(item.getKey()));
            reader.endMapItem();
            break;
         default:

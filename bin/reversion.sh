...
function copyXMLs() {
    ...
    XMLS=$(find . -name "*${OLDMAJOR}${SEPARATOR}${OLDMINOR}.xml" -print)
    for XML in $XMLS; do
    ...
    PROPERTIES_FILES=$(find . -name "*${OLDMAJOR}${SEPARATOR}${OLDMINOR}.properties" -print)
...
function addNamespace() {
    ...
    NAMESPACES=$(find . -name "*.namespaces" -print)
...
if [ "$OLDSCHEMAVERSION" != "$NEWSCHEMAVERSION" ] && [ "$PROCESS_SCHEMAS" = true ] ;  then
    ...
    CONFIGS=$(find server -regex '.*/src/test/resources/[^0-9].*xml')
    for CONFIG in $CONFIGS; do
         rewriteXML "$CONFIG" "$OLDSCHEMAMAJOR" "$OLDSCHEMAMINOR" "$NEWSCHEMAMAJOR" "$NEWSCHEMAMINOR"
    done

...
public interface PersistenceManager extends Lifecycle {
    ...
    * cache store will be used. Predicate is applied by the underlying loader in an attempt to improve performance.
    * <p>
    ...
    * will be used. Predicate is applied by the underlying loader in an attempt to improve performance.
    * <p>
   ...
   enum AccessMode implements Predicate<StoreConfiguration> {
      /**
       * The operation is performed in all {@link org.infinispan.persistence.spi.NonBlockingStore}.
       */
      BOTH {
         @Override
      ...
      /**
       * The operation is performed only in shared configured {@link org.infinispan.persistence.spi.NonBlockingStore}
       */
      SHARED {
         @Override
         public boolean test(StoreConfiguration configuration) {
      ...
      /**
       * The operation is performed only in non-shared {@link org.infinispan.persistence.spi.NonBlockingStore}
       */
      PRIVATE {
         @Override
         public boolean test(StoreConfiguration configuration) {
      ...
      /**
       * The operation is performed only in a {@link org.infinispan.persistence.spi.NonBlockingStore} that has async write behind.
       */
      ASYNC {
         @Override
         public boolean test(StoreConfiguration configuration) {
      ...
      /**
       * The operation is performed only in a {@link org.infinispan.persistence.spi.NonBlockingStore} that doesn't have async write behind.
       */
      NOT_ASYNC {
         @Override
         public boolean test(StoreConfiguration configuration) {
   ...
   /**
    * Write to all stores that are not transactional. A store is considered transactional if all the following are true:
    *
    * <ul>
   ...
   /**
    * @return true if no {@link NonBlockingStore} instances have been configured.
    */
   boolean isReadOnly();
}

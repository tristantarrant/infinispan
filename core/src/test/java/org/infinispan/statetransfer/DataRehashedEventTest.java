...
public class DataRehashedEventTest extends MultipleCacheManagersTest {
   ...
   public void testJoinAndLeave() {
      ...
      ConsistentHash ch1Node = advancedCache(0).getDistributionManager().getCacheTopology();
      ...
      ConsistentHash ch2Nodes = advancedCache(0).getDistributionManager().getCacheTopology();
      ...
      ConsistentHash ch3Nodes = advancedCache(0).getDistributionManager().getCacheTopology();
      rehashListener.waitForEvents(2);
      ...
      ConsistentHash chAfterLeave = advancedCache(0).getDistributionManager().getCacheTopology();
      rehashListener.waitForEvents(2);
      events = rehashListener.removeEvents();
      assertEquals(events.size(), 2);
      pre = events.get(0);

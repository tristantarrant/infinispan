...
public class ThreeWaySplitAndMergeTest extends BasePartitionHandlingTest {
   ...
   private void testSplitAndMerge(PartitionDescriptor p0, PartitionDescriptor p1, PartitionDescriptor p2) throws Exception {
      ...
      assertEquals(new HashSet<>(advancedCache(p0.node(0)).getDistributionManager().getCacheTopology().getMembers()), members);
      assertEquals(new HashSet<>(advancedCache(p0.node(1)).getDistributionManager().getCacheTopology().getMembers()), members);
      assertEquals(new HashSet<>(advancedCache(p1.node(0)).getDistributionManager().getCacheTopology().getMembers()), members);

      if (partitionHandling == PartitionHandling.DENY_READ_WRITES)
         partition(1).assertKeysNotAvailableForRead(k0, k1, k2, k3);

      members = new HashSet<>(Arrays.asList(new Address[]{address(0), address(1), address(2), address(3)}));
      assertEquals(new HashSet<>(advancedCache(p2.node(0)).getDistributionManager().getCacheTopology().getMembers()), members);

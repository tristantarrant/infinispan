...
public class HotRodConnectionMetadataTest extends HotRodSingleNodeTest {
   ...
   public void testProtocolVersionSetOnLatestVersion(Method m) {
      // Verify the most common case: a client using the latest protocol version
      try (HotRodClient clientLatest = new HotRodClient("127.0.0.1", hotRodServer.getPort(), cacheName,
            HotRodVersion.LATEST.getVersion(), false)) {
         assertStatus(clientLatest.put(k(m), 0, 0, v(m)), Success);

         ConnectionMetadata metadata = getConnectionMetadata(clientLatest);
         assertEquals(HotRodVersion.LATEST.toString(), metadata.protocolVersion());
      }
   }

...
/**
 * Function to get values by reverse rank from a T-Digest using FunctionalMap.
 * Used by TDIGEST.BYREVRANK command.
 *
 * @since 16.2
 */
@ProtoTypeId(ProtoStreamTypeIds.RESP_TDIGEST_BYREVRANK_FUNCTION)
public final class TDigestByRevRankFunction
      implements SerializableFunction<EntryView.ReadEntryView<byte[], Object>, List<Double>> {

   private final List<Long> ranks;

   @ProtoFactory
   public TDigestByRevRankFunction(List<Long> ranks) {
      this.ranks = ranks;
   }
   ...
   public List<Double> apply(EntryView.ReadEntryView<byte[], Object> view) {
      ...
      for (Long rank : ranks) {
         results.add(tdigest.byRevRank(rank));
      }
      return results;
   }
}

 ...
 * <p>
 * <b>Deprecated:</b> Use GEOSEARCH with FROMMEMBER instead.
 *
...
public class GEORADIUSBYMEMBER extends RespCommand implements Resp3Command {
   ...
   public CompletionStage<RespRequestHandler> perform(Resp3Handler handler,
      ...
      // GEORADIUSBYMEMBER key member radius unit [WITHCOORD] [WITHDIST] [WITHHASH] [COUNT count [ANY]] [ASC|DESC] [STORE key] [STOREDIST key]
      byte[] key = arguments.get(0);
      byte[] member = arguments.get(1);

      double radius;
      try {
         radius = ArgumentUtils.toDouble(arguments.get(2));
      } catch (NumberFormatException e) {
         handler.writer().valueNotAValidFloat();
         return handler.myStage();
      }

      GeoUnit unit = GeoUnit.parse(arguments.get(3));
      if (unit == null) {
      ...
      if (storeKey != null && (withCoord || withDist || withHash)) {
         handler.writer().customError("STORE option in GEORADIUSBYMEMBER is not compatible with WITHDIST, WITHHASH and WITHCOORD options");
         return handler.myStage();
      }

      EmbeddedMultimapSortedSetCache<byte[], byte[]> sortedSetCache = handler.getSortedSeMultimap();

      // Get center coordinates from member
      ...
      return sortedSetCache.score(key, member)
            .thenCompose(score -> {
               if (score == null) {
               ...
               double[] center = GeoHashUtil.decode(GeoHashUtil.scoreToGeohash(score));

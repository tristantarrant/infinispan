...
public class TDIGESTMERGE extends RespCommand implements Resp3Command {
   ...
   public CompletionStage<RespRequestHandler> perform(Resp3Handler handler, ChannelHandlerContext ctx,
      ...
      try {
         ...
         handler.writer().customError(ProbabilisticErrors.TDIGEST_ERROR_PARSING_NUMKEYS);
         return handler.myStage();
      }

      if (numKeys <= 0) {
         handler.writer().customError(ProbabilisticErrors.TDIGEST_NUMKEYS_POSITIVE);
         return handler.myStage();
      }
      ...
      if (idx < arguments.size()) {
         ...
         if ("COMPRESSION".equals(option) && idx + 1 < arguments.size()) {
            try {
               ...
               handler.writer().customError(ProbabilisticErrors.TDIGEST_ERROR_PARSING_COMPRESSION);
               return handler.myStage();
            }
            if (compression <= 0) {
               handler.writer().customError(ProbabilisticErrors.TDIGEST_COMPRESSION_POSITIVE);
               return handler.myStage();
            }
         }
      }

...
public final class CuckooFilter {
   ...
   public boolean add(byte[] item) {
      ...
      // Try direct insertion (no eviction) in all sub-filters
      for (SubFilter filter : subFilters) {
         if (filter.tryInsert(fingerprint, hash)) {
            itemsInserted++;
            return true;
         }
      }

      // Try to expand before eviction to avoid corrupting existing sub-filters
      if (expansion > 0) {
         SubFilter lastFilter = subFilters.get(subFilters.size() - 1);
         long newCapacity = roundToPowerOfTwo(lastFilter.getNumBuckets() * expansion);
         SubFilter newFilter = new SubFilter(newCapacity, bucketSize);
         if (newFilter.tryInsert(fingerprint, hash)) {
      ...
      // Last resort: eviction on the last sub-filter
      SubFilter lastFilter = subFilters.get(subFilters.size() - 1);
      if (lastFilter.add(fingerprint, hash, maxIterations)) {
         itemsInserted++;
         return true;
      }
   ...
   public static final class SubFilter {
      ...
      public boolean tryInsert(byte fingerprint, long hash) {
         ...
         return insertInBucket(i1, fingerprint) || insertInBucket(i2, fingerprint);
      }
      ...
      /**
       * Checking only i1 and i2 is sufficient because altIndex is its own inverse:
       * altIndex(altIndex(i, fp), fp) == i. Even after eviction chains, every item
       * remains in one of its two candidate buckets.
       */

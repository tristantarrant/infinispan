...
# Enable EPEL
#dnf install -y /tmp/infinispan/*.rpm
wget -q -O /tmp/infinispan/temurin-latest.tar.gz 'https://api.adoptium.net/v3/binary/latest/25/ea/linux/x64/jdk/hotspot/normal/eclipse?project=jdk'
...
tar xzf /tmp/infinispan/temurin-latest.tar.gz -C "$rootfs"/usr/lib/jvm
ln -s "$rootfs"/usr/lib/jvm/jdk*/bin/* "$rootfs"/usr/bin

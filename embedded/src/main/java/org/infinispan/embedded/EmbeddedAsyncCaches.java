...
public class EmbeddedAsyncCaches implements AsyncCaches {
   ...
   @Override
   public <K, V> CompletionStage<AsyncCache<K, V>> create(String name, CacheConfiguration cacheConfiguration, AdminFlag... flags) {
      CompletionStage<Cache<K, V>> cache = embedded.cacheManager.administration()
            .withFlags(EmbeddedSyncCaches.toAdminFlags(flags)).createCacheAsync(name, (Configuration) cacheConfiguration);
      return cache.thenApply(c -> new EmbeddedAsyncCache<>(embedded, c.getAdvancedCache()));
   }

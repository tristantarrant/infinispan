 ...
 * @since 16.3
 */
final class EmbeddedAsyncTransactionManager implements AsyncTransactionManager {
   private final TransactionManager transactionManager;

   EmbeddedAsyncTransactionManager(TransactionManager transactionManager) {
   ...
   @Override
   public EmbeddedAsyncTransactionManager rollbackOnly() {
      try {
   ...
   @Override
   public EmbeddedAsyncTransactionManager timeout(int seconds) {
      try {
   ...
   public AsyncTransaction getTransaction() {
      try {
         ...
         return tx == null ? null : new EmbeddedAsyncTransaction(tx);
      } catch (Exception e) {
         throw new TransactionException(e);
      }
   }
   ...
   public CompletionStage<AsyncTransaction> suspend() {
      try {
         ...
         return CompletableFuture.completedFuture(tx == null ? null : new EmbeddedAsyncTransaction(tx));
      } catch (Exception e) {
         return CompletableFuture.failedFuture(new TransactionException(e));
      }
   }

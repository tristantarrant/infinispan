...
/**
 * Embedded implementation of {@link AsyncCacheListener} that bridges the new builder-based
 * listener API to the internal {@link CacheNotifierImpl} infrastructure.
 *
 * @since 16.3
 */
public class EmbeddedAsyncCacheListener<K, V> extends AsyncCacheListener<K, V> {
   private final AdvancedCache<K, V> cache;

   public EmbeddedAsyncCacheListener(AdvancedCache<K, V> cache) {
      this.cache = cache;
   }
   ...
   private void registerIfPresent(CacheNotifierImpl<K, V> notifier, Object target, UUID identifier,
      ...
      Function<Event<K, V>, CompletionStage<Void>> wrappedFunction = event -> {
         if (event instanceof EventImpl) {
            ...
            return (CompletionStage<Void>) callback.apply(apiEvent);
         }
         return CompletableFuture.completedFuture(null);
      };
      FunctionalListenerInvocation<K, V> listenerInvocation =
            FunctionalListenerInvocation.async(target, wrappedFunction);
      CacheEntryListenerInvocation<K, V> invocation = new FunctionalCacheEntryListenerInvocation<>(
            listenerInvocation, annotationClass, onlyPrimary, clustered, identifier, false, observation);

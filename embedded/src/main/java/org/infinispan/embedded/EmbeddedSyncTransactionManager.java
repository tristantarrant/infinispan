 ...
 * @since 16.3
 */
final class EmbeddedSyncTransactionManager implements SyncTransactionManager {
   private final TransactionManager transactionManager;

   EmbeddedSyncTransactionManager(TransactionManager transactionManager) {
   ...
   @Override
   public EmbeddedSyncTransactionManager setRollbackOnly() {
      try {
   ...
   @Override
   public EmbeddedSyncTransactionManager setTransactionTimeout(int seconds) {
      try {
   ...
   public SyncTransaction getTransaction() {
      try {
         ...
         return tx == null ? null : new EmbeddedSyncTransaction(tx);
      } catch (Exception e) {
         throw new TransactionException(e);
      }
   }
